          ...
          <div className="w-full max-w-md rounded-2xl p-5 my-auto mx-4" style={{ background: "var(--card)" }}>
            ...
            <div className="flex gap-2">
              ...
              <button onClick={addBank} disabled={!newBankName.trim()}
                className="px-4 py-2.5 rounded-xl text-sm font-semibold text-white" style={{ background: newBankName.trim() ? "var(--foreground)" : "var(--muted)", color: newBankName.trim() ? "white" : "var(--muted-foreground)" }}>Tambah</button>
            </div>
          </div>
        </div>
      )}
