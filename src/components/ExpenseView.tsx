...
export default function ExpenseView({ expenses, stores, employees = [], currentUser, onSave, onDelete, canAdd = true, canEdit = true, canDelete = true }: Props) {
  ...
  const handleExport = () => {
    ...
    showToast("File Excel pengeluaran berhasil diunduh");
  };
    ...
    <div className="flex flex-col h-full overflow-y-auto lg:overflow-hidden">
      ...
      {lightbox && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4" style={{ background: "rgba(0,0,0,0.8)" }} onClick={() => setLightbox("")}>
          <img src={assetUrl(lightbox)} alt="Bukti pengeluaran" className="max-h-[85vh] max-w-[95vw] rounded-2xl object-contain" />
          <button className="absolute top-4 right-4 w-9 h-9 rounded-full bg-white/20 text-white flex items-center justify-center" aria-label="Tutup">
        ...
        <div className="fixed inset-0 z-40 flex items-start justify-center overflow-y-auto" style={{ background: "rgba(0,0,0,0.5)" }}>
          <div className="w-full max-w-lg rounded-2xl overflow-hidden my-6 mx-4" style={{ background: "var(--card)" }}>
            ...
            <div className="p-5 flex flex-col gap-4">
              ...
              <div>
                <div className="text-xs font-medium mb-1.5" style={{ color: "var(--muted-foreground)" }}>Bukti Foto Transaksi</div>
                <div className="flex flex-col gap-2">
                  {editing.photo ? (
                    ...
                    <button onClick={() => photoRef.current?.click()}
                      ...
                      Ambil Foto Bukti (Kamera) / Pilih Dari Galeri
                    </button>
                  )}
      ...
      <div className="flex flex-col min-w-0 lg:flex-1 lg:overflow-hidden">
        <div className="px-4 sm:px-6 py-4 border-b shrink-0" style={{ borderColor: "var(--border)", background: "var(--background)" }}>
          <div className="flex items-center justify-between mb-3 flex-wrap gap-3">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              {canAdd && (
                <button onClick={() => { setEditing({ ...empty(), createdByName: currentUser?.name ?? "" }); setIsNew(true); }}
        ...
        <div className="lg:flex-1 lg:overflow-y-auto p-4 sm:p-6">
          <div className="flex flex-col gap-3">
            {pageItems.length === 0 && (
              <EmptyState icon="💸" title={expenses.length === 0 ? "Belum ada pengeluaran" : "Tidak ada catatan sesuai filter"} hint={expenses.length === 0 ? "Catatan pengeluaran operasional akan muncul di sini." : "Coba ubah rentang tanggal atau kata kunci pencarian."} />
            )}
            {pageItems.map(e => (
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="flex items-start gap-3 flex-1 min-w-0">
                    <div className="w-10 h-10 rounded-xl flex items-center justify-center shrink-0" style={{ background: "#fef3c7", color: "#d97706" }}>
                      <svg width="18" height="18" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}><path strokeLinecap="round" strokeLinejoin="round" d="M20 7l-8-4-8 4m16 0l-8 4 8 4m0-8v10l-8 4-8-4V7m16 10l-8 4m8-4l-8-4" /></svg>
                    </div>
                    <div className="flex-1 min-w-0">
                      ...
                      {e.photo && (
                        <img src={assetUrl(e.photo)} alt="Bukti pengeluaran" onClick={() => setLightbox(e.photo ?? "")}
